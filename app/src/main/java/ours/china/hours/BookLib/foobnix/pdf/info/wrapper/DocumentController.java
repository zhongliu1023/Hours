...
import ours.china.hours.BookLib.foobnix.android.utils.MyMath;
...
public abstract class DocumentController {
    ...
    public static void showFullScreenPopup(Activity a, View v, final IntegerResponse response, int currentMode) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && a.getWindow().getDecorView().getRootWindowInsets().getDisplayCutout() != null) {
            ...
            for (final int id : ids)
                ...
                if (id == AppState.FULL_SCREEN_FULLSCREEN_CUTOUT && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    ...
                    popup.getMenu().add(DocumentController.getFullScreenName(a, id)).setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
                        @Override
